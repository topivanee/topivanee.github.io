...
// =========================
// CODE EXPLORER MODAL LOGIC
// =========================
const codeModal = document.getElementById("code-modal");
const closeModalBtn = document.getElementById("close-modal-btn");
const copyCodeBtn = document.getElementById("copy-code-btn");
const scriptTreeList = document.getElementById("script-tree-list");
...
document.querySelectorAll(".view-code-btn").forEach(btn => {
    btn.addEventListener("click", () => {
        const gameTitle = btn.getAttribute("data-game") || "ROBLOX STUDIO SOURCE EXPLORER";
        modalGameName.textContent = gameTitle.toUpperCase();
        
        try {
            currentScripts = JSON.parse(btn.getAttribute("data-scripts"));
        } catch (e) {
            currentScripts = [];
        }

        if (currentScripts.length > 0) {
            renderScriptList(currentScripts);
            loadScript(0);
...
function renderScriptList(scripts) {
    scriptTreeList.innerHTML = "";
    scripts.forEach((script, index) => {
        const li = document.createElement("li");
        li.innerHTML = `<i class="fa-solid fa-file-code"></i> ${script.path}`;
        if (index === 0) li.classList.add("active");
        
        li.addEventListener("click", () => {
            document.querySelectorAll("#script-tree-list li").forEach(el => el.classList.remove("active"));
            li.classList.add("active");
            loadScript(index);
        });

        scriptTreeList.appendChild(li);
    });
}

function loadScript(index) {
    activeScriptIndex = index;
    const selected = currentScripts[index];
    if (!selected) return;

    modalFilePath.innerHTML = `<i class="fa-solid fa-file-code"></i> ${selected.path}`;
    codeContent.textContent = selected.code;
...
if (copyCodeBtn) {
    copyCodeBtn.addEventListener("click", () => {
        if (currentScripts[activeScriptIndex]) {
            navigator.clipboard.writeText(currentScripts[activeScriptIndex].code);
...
if (scriptSearchInput) {
    scriptSearchInput.addEventListener("input", (e) => {
        ...
        const items = scriptTreeList.querySelectorAll("li");
        items.forEach(item => {
